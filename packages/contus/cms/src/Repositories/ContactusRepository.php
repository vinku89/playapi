...
class ContactusRepository extends BaseRepository {
     ...
     */
    public function __construct(Contactus $contactUs,Customer $customer, EmailTemplatesRepository $emailTemplates, NotificationRepository $notificationRepository) {
        parent::__construct ();
    ...
    public function addContactus($id = null) {
        ...
        if (!empty($user) && count($user) > 0) {
            $this->email = $this->email->fetchEmailTemplate('contact_us');
            $this->email->subject = str_replace(['##SITE_NAME##'], [config ()->get ( 'settings.general-settings.site-settings.site_name' )], $this->email->subject);
            $this->email->content = str_replace([ '##USERNAME##','##NAME##','##EMAIL##','##CONTACTNUMBER##','##MESSAGE##' ], [ 'Admin',$this->request->input('name'),$this->request->input('email'),$this->request->input('phone'),$this->request->input('message') ], $this->email->content);
