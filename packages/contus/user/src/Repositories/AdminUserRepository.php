       ...
       $this->uploadRepository->defineRepositoryFileRule($this);
        ...
        if ($adminUser->save()) {
            if (empty($id)) {
                ...
                $modifierName = Auth::user()->name;
                $this->email = $this->email->fetchEmailTemplate('customer-admin-account-update');
                $this->email->subject = str_replace(['##SITE_NAME##'], [$this->siteName], $this->email->subject);
                $this->email->content = str_replace(['##GREETING_NAME##','##DATE##','##MODIFIER_NAME##' ], [ $this->request->name,'\''.date('d-m-Y h:i:s').'\'',$modifierName], $this->email->content);
            }
