...
class RecentlyViewedVideoRepository extends BaseRepository {
    ...
    public function addRecentVideos($videoid) {
        $selectedVideos = Video::where ( $this->getKeySlugorId (),  $videoid )->first();
        if($selectedVideos){
            $this->authUser->recentlyViewed ()->detach ( $selectedVideos->id );
            $this->authUser->recentlyViewed ()->attach ( $selectedVideos->id );
            return true;
        }else {
            return false;
        }
