                        ...
                        <form name="editCustomer" novalidate
                            ...
                            <div flow-object="existingFlowObject" flow-init flow-file-added="!!{png:1,gif:1,jpg:1,jpeg:1}[$file.getExtension()]" flow-files-submitted="$flow.upload()">
                                <div class="">
                                   ...
                                   <hr class="soften"/>
                               <div>
                              <div class="thumbnail upload-img" ng-hide="$flow.files.length">
                              <img   ng-src="@{{profile.profile_picture}}"   src="{{$cdnUrl('images/user.png')}}" />
    						</div>
    						<div class="thumbnail upload-img"  ng-show="$flow.files.length">
    						...
    						<div class="cs-changetext-links">
      						...
      						<button class="btn btn-primary" ng-show="$flow.files.length" flow-btn flow-attrs="{accept:'image/*'}">Change</button>
      						<button class="btn btn-danger" ng-show="$flow.files.length" ng-click="$flow.cancel()	;clearProfilepic()">Remove</button>
   							 </div>
