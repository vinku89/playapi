        ...
        <form name="userForm" method="POST" data-base-validator data-ng-submit="usrCtrl.save($event, usrCtrl.user.id)" enctype="multipart/form-data">
            ...
            <div class="video_form add_form">
                ...
                <div class="form-group" data-ng-class="{'has-error': errors.email.has}">
                    ...
                    <input data-ng-if="usrCtrl.user.id" readonly type="text" name="email" data-ng-model="usrCtrl.user.email" class="form-control" placeholder="{{trans('customer::customer.email_placeholder')}}" value="{{old('email')}}" />
                    <input data-ng-if="!usrCtrl.user.id" type="text" name="email" data-ng-model="usrCtrl.user.email" class="form-control" placeholder="{{trans('customer::customer.email_placeholder')}}" value="{{old('email')}}" />
                ...
                <div class="form-group cs-newstudent" data-ng-class="{'has-error': errors.exam.has}">
                    <label class="control-label">{{trans('video::videos.exam')}} <span class="asterisk">*</span></label>
                    <label for="@{{exam.id}}" ng-repeat="exam in exams">
                        <input type="checkbox" value="@{{exam.id}}" id="@{{exam.id}}" ng-click="selectexam(exam.id)" ng-checked="examSelection.indexOf(exam.id) > -1">
                        @{{exam.title}}
                    </label>
