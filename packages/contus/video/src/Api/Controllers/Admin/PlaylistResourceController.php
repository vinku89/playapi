...
class PlaylistResourceController extends ApiController {
    ...
    public function store() {
        ...
        if ($save) {
         $isPlaylistAdd= true;
         $this->request->session ()->flash ( StringLiterals::SUCCESS, trans ( 'video::playlist.created.added' ) );
        }
        
        return ($isPlaylistAdd) ? $this->getSuccessJsonResponse ( [
          StringLiterals::STATUS => 'success',
          StringLiterals::MESSAGE => trans ( 'video::playlist.created.added' )
        ] ) : $this->getErrorJsonResponse ( [
          [
            StringLiterals::STATUS => 'error',
            StringLiterals::MESSAGE => trans ( 'video::playlist.error.added' )
          ]
        ] );
