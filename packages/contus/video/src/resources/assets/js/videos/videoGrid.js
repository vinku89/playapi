...
var VideoGridController = ['flowFactory', '$scope', 'requestFactory', '$window', '$sce', '$timeout', '$compile', '$interval', function(flowFactory, scope, requestFactory, $window, $sce, $timeout, $compile, $interval) {
  ...
  this.setupGoogleDriveUploader = function() {
    ...
    this.googleDriveUploader.initiate({
      id: 'google_drive_upload_button',
      afterUpload: function(response) {
        self.video.video_details = response;
        self.saveSingleVideo();
      },
  ...
  this.editVideoForm = function(videoDetails) {
    ...
    if (videoDetails.job_status == 'Complete') {
      ...
      myPlayer.src([{
        type: "video/mp4",
        src: $sce.trustAsResourceUrl(videoDetails.transcodedvideos[0].video_url)
      }]);
      ...
      canvasData = cropper.getCroppedCanvas().toBlob(function(blob) {
        ...
        $.ajax($('meta[name="base-api-url"]').attr('content') + '/videos/thumbnail', {
          ...
          success(data) {
            ...
            self.editVideo.selected_thumb = data.info;
...
function googleDriveUploader() {
  ...
  this.initializeFineUploader = function() {
    window.fineUploaderGoogleDrive = new qq.FineUploaderBasic({
      autoUpload: false,
      debug: true,
      element: document.getElementById('file_drop_area'),
      request: {
        endpoint: window.VPlay.route.siteUrl + '/api/admin/videos/handle-fine-uploader'
      },
      deleteFile: {
        enabled: true,
        endpoint: window.VPlay.route.siteUrl + '/api/admin/videos/handle-fine-uploader'
      ...
      chunking: {
        ...
        success: {
          endpoint: window.VPlay.route.siteUrl + '/api/admin/videos/handle-fine-uploader?done'
        }
      },
      resume: {
        enabled: true
  ...
  this.onApiLoad = function() {
    gapi.load('auth', {
      'callback': self.onAuthApiLoad
    });
    gapi.load('picker', {
      'callback': self.onPickerApiLoad
    });
  }
    ...
    window.fineUploader = new qq.FineUploaderBasic({
      ...
      callbacks: {
        ...
        onProgress: function(id, name, uploadedBytes, totalBytes) {
          ...
          $('#published_on' + self.currentFileCount).datepicker({
            format: "dd-mm-yyyy",
            viewMode: 'years',
            autoclose: true
          });
