...
videoDetail.controller('VideoDetailController', ['flowFactory', '$window', '$scope', '$rootScope', 'requestFactory', '$timeout', '$sce', function(flowFactory, win, scope, $rootScope, requestFactory, $timeout, strictContextual) {
  ...
  this.fetchData = function(id) {
    requestFactory.get(requestFactory.getUrl('videos/video-to-edit/' + id), function(response) {
      ...
      this.editVideo.thumbnail_path = videoDetails.thumbnail_path;
      this.editVideo.thumbnail = videoDetails.thumbnail_image;
      this.editVideo.poster_image = videoDetails.poster_image;
  ...
  /*
   * Function to delete admin video edit page thumbnail.
   */
  this.deleteSinglethumbnail = function(id) {
  ...
  this.confirmDeleteVideos = function() {
    if (scope.deleteParams.length > 0) {
      self.deleteThumbnail(scope.deleteParams);
      scope.videoConfirmationDeleteBox = false;
      scope.deleteParams = '';
    } else {
      scope.videoConfirmationDeleteBox = false;
  ...
  this.deleteThumbnail = function(id) {
    requestFactory.post(requestFactory.getUrl('videos/delete-thumbnail/' + id), this.editVideo, function() {
  ...
  this.showPosterModel = function() {
    ...
    this.VideoPosterUploader.initate({
      file: 'poster-images',
      button: 'poster-image-upload-proceed',
      progress: 'poster-progress',
      afterUpload: function(response) {
        this.removeProgressListByIndex(response.file.index);
        self.editVideo.posters.push({
          temp: response.info
        });
        self.editVideo.newPosters.push({
          file: response.file
        });
        if (!scope.$$phase) {
          scope.$apply();
  ...
  this.showCastModel = function(cast, index) {
    ...
    this.VideoCastImageUploader.initate({
      file: 'cast-images',
      button: 'cast-image-upload-proceed',
