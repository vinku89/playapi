<section class="breadcrumbs-section">
    <div class="container">
        <div class="row">
            <nav class="breadcrumb" ng-if="(live.total)?true:false">
                <a class="breadcrumb-item" href="{{URL::to('/')}}">Home</a>
                <a class="breadcrumb-item" ui-sref="livevideos">
            ...
            <nav class="breadcrumb" ng-if="(playlist.name)?true:false">
                ...
                <a class="breadcrumb-item" ui-sref="@{{(playlist.name)?'playlist':'categoryvideos({slug:category.parent_category.parent_category.slug})'}}">
                    <span>@{{(playlist.name)?playlist.name:category.parent_category.parent_category.title}}</span>
                </a>
            </nav>
            <nav class="breadcrumb" ng-if="(playlist.name || live.total)?false:true">
                <a class="breadcrumb-item" href="{{URL::to('/')}}">Home</a>
                <a class="breadcrumb-item" ui-sref="@{{(playlist.name)?'playlist':'category'}}">
                    <span>Browse Videos </span>
                </a>
                ...
                <a class="breadcrumb-item" ui-sref="categoryvideos({slug:category.parent_category.parent_category.slug})">
                    <span>@{{(playlist.name)?playlist.name:category.parent_category.title}}</span>
                </a>
                <a class="breadcrumb-item" ui-sref="categorysection({category:category.parent_category.parent_category.slug,slug:category.slug})">
                    <span>@{{(playlist.name)?playlist.name:category.title}}</span>
                </a>
            </nav>
        </div>
    </div>
                ...
                <div class="row">
                    <div class="panel panel-default sub-widgets related-videos">
                        ...
                        <div class="panel-body scrollfinderrelated" custom-scroll="{ 'autoHide': false }">
                            <div class="media" ng-repeat="video in related.data" ng-class="{'active':passclass(video.slug)}">
                                <div class="media-left">
                                    <a href="javascript:;" ng-click="passroute(video.slug)" title="@{{video.title}}">
                                            <span  ng-if="(videos.youtube_id)?false:true"  ng-show="videos.is_demo" class="demo-label">demo</span>
                                            <img alt="64x64" class="media-object notification-img" ng-src="@{{(video.thumbnail_image)?video.thumbnail_image:video.selected_thumb}}" src="{{$cdnUrl('images/no-preview.png')}}" data-holder-rendered="true">
                                            <span class="time-label" ng-show="!(videos.liveStatus)"> @{{video.video_duration}}</span>
                                        </a>
                                </div>
                                <div class="media-body">
                                    <h4 class="media-heading">
                                        <a href="javascript:;" ng-click="passroute(video.slug)" title=" @{{video.title}}"> @{{video.title}}</a>
                                    </h4>
