...
class VideoRepository extends BaseRepository implements IVideoRepository
{
     ...
     */
    public function __construct(AWSUploadRepository $awsRepository, UploadRepository $uploadRepository, VideoCountriesRepository $videoCountriesRepository, VideoCastRepository $videoCastRepository, CommentsRepository $commentRepository, QuestionsRepository $questionrepository)
    {
        parent::__construct();
    ...
    public function getVideoEditRules()
    {
         ...
         */
        $this->setRules([StringLiterals::TITLE => StringLiterals::REQUIRED, StringLiterals::CATEGORYIDS => StringLiterals::REQUIREDINTEGER, 'description' => StringLiterals::REQUIRED, 'is_featured' => StringLiterals::REQUIREDINTEGER, StringLiterals::ISACTIVE => StringLiterals::REQUIREDINTEGER, StringLiterals::TRAILER => 'url', 'presenter' => 'required']);
    ...
    protected function updateGridQuery($builder)
    {
        ...
        if (!empty($filters)) {
            foreach ($filters as $value) {
                if ($value =="normal_videos") {
                    $builder->where(StringLiterals::IS_ARCHIVED, 0)->where('is_live', 0);
                } else {
                    $builder->where(StringLiterals::IS_ARCHIVED, 0)->where('is_live', 0);
                }
            }
        ...
        if (!$filters) {
            return $builder->where('is_live', 0)->where(StringLiterals::IS_ARCHIVED, 0);
        } else {
            return $builder;
        }
    }
